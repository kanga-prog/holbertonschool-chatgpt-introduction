...
class Minesweeper:
    def __init__(self, width=10, height=10, mines=10):
        ...
        self.total_non_mine_cells = width * height - mines  # Total non-mine cells

    def print_board(self, reveal=False):
        clear_screen()
        print('  ' + ' '.join(str(i) for i in range(self.width)))
        for y in range(self.height):
            print(f'{y:2}', end=' ')  # Ensure row number is aligned
    ...
    def check_win(self):
        ...
        return revealed_non_mines == self.total_non_mine_cells
